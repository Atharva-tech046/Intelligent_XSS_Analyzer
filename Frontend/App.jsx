...
import { Shield, ShieldAlert, Terminal, Activity, Database, Server, SearchCode, Cpu, X, Play, RefreshCw, XCircle, ChevronDown, Lock, Zap, ArrowRight, User, Key, Bot, Timer, ShieldCheck, BarChart2, CheckCircle, ChevronUp, Clock } from 'lucide-react';
import './App.css';
...
function App() {
  ...
  const handleScan = async () => {
    ...
    try {
      ...
      if (isVulnerable) {
        addLog(`Engine identified critical vulnerabilities in ${duration}s.`, "error");
        setFinding(data[0]);
  ...
  const handleSchedule = async () => {
    ...
    addLog(`Configuring autonomous DevSecOps agent for ${targetUrl}...`, "system");

    try {
      const response = await fetch('http://127.0.0.1:5000/api/schedule', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ url: targetUrl, scan_type: scanType, frequency: scheduleFreq })
      });
      
      const data = await response.json();
      addLog(`SUCCESS: Background Agent deployed. Frequency: ${scheduleFreq.toUpperCase()}.`, "success");
      addLog(`You can now close the browser. The Python backend will scan autonomously.`, "normal");
    } catch (error) {
      addLog(`Network Error: Failed to deploy background agent.`, "error");
    } finally {
      ...
      <header className="top-nav">
        ...
        <div className="control-bar">
          <div className="status-indicator glass-panel">
            <div className={`status-dot ${isScanning || isScheduling ? 'scanning' : 'idle'}`}></div>
            {isScanning ? 'Engine Active' : isScheduling ? 'Deploying Agent' : 'System Ready'}
          </div>
          ...
          <div className="input-group">
            ...
            <select className="select-control" value={scheduleFreq} onChange={(e) => setScheduleFreq(e.target.value)} disabled={isScanning || isScheduling} style={{ width: '130px', borderLeft: '1px solid var(--bg-border)' }}>
              <option value="demo">Demo (1 Min)</option>
              <option value="hourly">Hourly</option>
              <option value="daily">Daily</option>
            </select>
          </div>
          
          <button className="btn btn-secondary btn-premium" onClick={handleSchedule} disabled={isScanning || isScheduling || !targetUrl}>
            {isScheduling ? <RefreshCw size={18} className="animate-spin" /> : <Clock size={18} />}
            Schedule Agent
          </button>
          
          <button className="btn btn-primary btn-premium" onClick={handleScan} disabled={isScanning || isScheduling || !targetUrl}>
